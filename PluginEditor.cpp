...
OsctestAudioProcessorEditor::OsctestAudioProcessorEditor (OsctestAudioProcessor& p)
    : AudioProcessorEditor (&p), processor (p), envGui(p), osc1Gui(p, 1), osc2Gui(p, 2), filterGui(p)
{
	masterGainAttachment = std::make_unique<AudioProcessorValueTreeState::SliderAttachment>(processor.treeState, "masterGain", masterGainKnob);
...
void OsctestAudioProcessorEditor::paint (Graphics& g)
{
	...
	g.fillRect(13, 60, 290, 270);							//OSC1
	g.fillRect(356, 60, 290, 270);							//FILTER
	g.fillRect(695, 60, 290, 270);							//OSC2
...
{
	//oscTypeBox.setBounds (10, 10, 150, 25);
	masterGainKnob.setBounds(949, 6, 34, 34);
	envGui.setBounds(13, 350, 462, 215);
	osc1Gui.setBounds(13, 60, 290, 270);
	osc2Gui.setBounds(695, 60, 290, 270);
	filterGui.setBounds(356, 60, 290, 270);
